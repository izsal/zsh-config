...
var require_tmp = __commonJS({
  "node_modules/tmp/lib/tmp.js"(exports, module2) {
    ...
    function _prepareTmpDirRemoveCallback(name, opts, sync) {
      const removeFunction = opts.unsafeCleanup ? rimraf2 : fs5.rmdir.bind(fs5);
...
function activate(context, outputChannel) {
  ...
  context.subscriptions.push(
    import_coc.languages.registerCodeActionProvider(documentSelector, new DoqCodeActionProvider(outputChannel), "pydocstring")
  );
}
var DoqCodeActionProvider = class {
  ...
  async provideCodeActions(document, range, context) {
    ...
    if (this.lineRange(range) || this.selectedRange(doc, range)) {
      ...
      const resolveRange = import_coc.Range.create(
  ...
  lineRange(range) {
    return (
      ...
      range.start.line + 1 === range.end.line && range.start.character === 0 && range.end.character === 0 || /** Older */
...
var DOQ_VERSION = getPackageVersion("doq");
...
async function doqInstall(pythonCommand, context) {
  ...
  try {
    import_coc2.window.showInformationMessage(`Install doq...`);
    await exec(
      `"${pythonCommand}" -m venv ${pathVenv} && ${pathVenvPython} -m pip install -U pip doq==${DOQ_VERSION}`
    );
    statusItem.hide();
    import_coc2.window.showInformationMessage(`doq: installed!`);
  } catch (error) {
    statusItem.hide();
    import_coc2.window.showErrorMessage(`doq: install failed. | ${error}`);
    throw new Error();
  }
}

// src/commands/install.ts
function activate2(context, builtinInstallPythonCommand) {
  const enableInstallPrompt = import_coc3.workspace.getConfiguration("pydocstring").get("enableInstallPrompt", true);
  context.subscriptions.push(
    import_coc3.commands.registerCommand("pydocstring.install", async () => {
      if (builtinInstallPythonCommand) {
        await installWrapper(context, builtinInstallPythonCommand, enableInstallPrompt);
      } else {
        import_coc3.window.showErrorMessage("python3/python command not found");
      }
    })
  );
}
async function installWrapper(context, pythonCommand, isPrompt) {
  if (isPrompt) {
...
function getDoqPath(context) {
  let doqPath = import_coc4.workspace.getConfiguration("pydocstring").get("doqPath", "");
  if (!doqPath) {
    const whichDoq = whichCmd("doq");
    if (whichDoq) {
      doqPath = whichDoq;
    } else if (import_fs2.default.existsSync(import_path3.default.join(context.storagePath, "doq", "venv", "Scripts", "doq.exe")) || import_fs2.default.existsSync(import_path3.default.join(context.storagePath, "doq", "venv", "bin", "doq"))) {
      if (process.platform === "win32") {
...
var WORKSPACE_PYPROJECT_TOML_PATH = import_path4.default.join(import_coc5.workspace.root, "pyproject.toml");
var WORKSPACE_SETUP_CFG_PATH = import_path4.default.join(import_coc5.workspace.root, "setup.cfg");
async function doFormat(context, outputChannel, document, range) {
  if (document.languageId !== "python") {
    throw "doq cannot run, not a python file";
  }
  const extensionConfig = import_coc5.workspace.getConfiguration("pydocstring");
  const formatterOption = extensionConfig.get("formatter", "sphinx");
  const templatePath = import_coc5.workspace.expand(extensionConfig.get("templatePath", ""));
  ...
  const args = [];
  const opts = { cwd: import_path4.default.dirname(fileName) };
  args.push("-w");
  args.push("--formatter", formatterOption);
  if (templatePath && import_fs3.default.existsSync(templatePath) && import_fs3.default.existsSync(import_path4.default.join(templatePath, "class.txt")) && import_fs3.default.existsSync(import_path4.default.join(templatePath, "def.txt")) && import_fs3.default.existsSync(import_path4.default.join(templatePath, "noarg.txt"))) {
    if (templatePath.startsWith("/")) {
      args.push("--template_path", templatePath);
    } else {
      args.push("--template_path", import_path4.default.join(import_coc5.workspace.root, templatePath));
    }
  }
  ...
  import_fs3.default.writeFileSync(tmpFile.name, text);
...
`);
  return new Promise(function(resolve) {
    import_child_process2.default.execFile(doqPath, [...args, tmpFile.name], opts, function(err) {
      if (err) {
        tmpFile.removeCallback();
        if (err.code === "ENOENT") {
          import_coc5.window.showErrorMessage("Unable to find the doq tool.");
          throw err;
        }
        import_coc5.window.showErrorMessage("There was an error while running doq.");
        throw err;
      }
      const text2 = import_fs3.default.readFileSync(tmpFile.name, "utf-8");
      tmpFile.removeCallback();
...
function getDoqTemplatePathFromConfigFile() {
  var _a, _b, _c;
  let templatePath;
  if (import_fs3.default.existsSync(WORKSPACE_PYPROJECT_TOML_PATH)) {
    try {
      const fileStr = import_fs3.default.readFileSync(WORKSPACE_PYPROJECT_TOML_PATH, { encoding: "utf-8" });
      const data = import_toml2.default.parse(fileStr);
      if ((_b = (_a = data.tool) == null ? void 0 : _a.doq) == null ? void 0 : _b.template_path) {
        templatePath = data.tool.doq.template_path;
      }
    } catch (e) {
      import_coc5.window.showErrorMessage("Failed to load TOML file. There may be a syntax error.");
      throw e;
    }
  } else if (import_fs3.default.existsSync(WORKSPACE_SETUP_CFG_PATH)) {
    try {
      const fileStr = import_fs3.default.readFileSync(WORKSPACE_SETUP_CFG_PATH, { encoding: "utf-8" });
      const data = import_ini.default.parse(fileStr);
      if ((_c = data.doq) == null ? void 0 : _c.template_path) {
        templatePath = data.doq.template_path;
      }
    } catch (e) {
      import_coc5.window.showErrorMessage("Failed to load INI file. There may be a syntax error.");
      throw e;
    }
  }
  if (templatePath) {
    if (templatePath.startsWith("/")) {
      ...
      return import_path4.default.join(import_coc5.workspace.root, templatePath);
    }
  }
  return templatePath;
}
...
// src/commands/runAction.ts
function activate3(context, outputChannel) {
  context.subscriptions.push(
    import_coc6.commands.registerCommand(
      ...
      async (document, range) => {
        ...
        if (!range) {
          ...
          edits = [import_coc6.TextEdit.replace(range, code)];
          if (edits) {
...
function activate4(context, outputChannel) {
  context.subscriptions.push(
    import_coc7.commands.registerCommand("pydocstring.runFile", async () => {
...
function activate5(context, outputChannel) {
  context.subscriptions.push(
    import_coc8.commands.registerCommand("pydocstring.showOutput", () => {
      outputChannel.show();
    })
  );
}
...
// src/index.ts
async function activate6(context) {
  if (!import_coc9.workspace.getConfiguration("pydocstring").get("enable", true))
    return;
  const outputChannel = import_coc9.window.createOutputChannel("pydocstring");
  activate5(context, outputChannel);
  const extensionStoragePath = context.storagePath;
  if (!import_fs4.default.existsSync(extensionStoragePath)) {
    import_fs4.default.mkdirSync(extensionStoragePath);
  }
  const doqPath = getDoqPath(context);
  let builtinInstallPythonCommand = import_coc9.workspace.getConfiguration("pydocstring").get("builtin.pythonPath", "");
  if (!builtinInstallPythonCommand) {
    const isRealpath = true;
    builtinInstallPythonCommand = getPythonCommand(isRealpath);
  }
  activate2(context, builtinInstallPythonCommand);
  if (!doqPath) {
    if (builtinInstallPythonCommand) {
      import_coc9.commands.executeCommand("pydocstring.install");
    } else {
      import_coc9.window.showErrorMessage("python3/python command not found");
    }
  }
  if (!doqPath)
    return;
  activate4(context, outputChannel);
  activate3(context, outputChannel);
